...
class HyBinance(SignalTemplate):
    def __init__(self, weight, ticker, signal_update_frequency_seconds, window_size=5, buy_threshold=-1, buy_exit_threshold=-1):
        super().__init__(weight, ticker, signal_update_frequency_seconds)
        self.window_size = window_size
        self.hyperliquid_window = deque([], maxlen=2)
        self.binance_window = deque([], maxlen=2)
    ...
    def initialize_window(self):
        try:
            ...
            df_binance = self.datafeed.fetch_ohlcv(binance, self.binance_symbol, TIMEFRAME, START_DATE, END_DATE)

            hyperliquid_window = df_hyperliquid['close'].apply(lambda x:int(x))
            binance_window = df_binance['close'].apply(lambda x:int(x))

            self.hyperliquid_window.extend(hyperliquid_window.to_list()[-2:])
            self.binance_window.extend(binance_window.to_list()[-2:])

            self.spread_window.extend((binance_window.diff().dropna() - hyperliquid_window.diff().dropna()).to_list())
